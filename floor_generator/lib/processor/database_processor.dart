...
class DatabaseProcessor extends Processor<Database> {
  ...
  List<DaoGetter> _getDaoGetters(
    ...
    return _classElement.fields2.where(_isDao).map((field) {
      final classElement = field.type.element3 as ClassElement2;
  ...
  Set<TypeConverter> _getAllTypeConverters(
    ...
    final fieldTypeConverters =
        queryables.expand((queryable) => queryable.fields).mapNotNull((field) => field.typeConverter).toSet();

    return daoQueryMethodTypeConverters + daoTypeConverters + fieldTypeConverters;
  }

  bool _isEntity(final ClassElement2 classElement) {
    return classElement.hasAnnotation(annotations.Entity) && !classElement.isAbstract;
  }

  bool _isView(final ClassElement2 classElement) {
    return classElement.hasAnnotation(annotations.DatabaseView) && !classElement.isAbstract;
  }
}
